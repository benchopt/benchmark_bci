...
class tWDA(BaseEstimator, ClassifierMixin, TransformerMixin):
    ...
    rmt: bool, default=False
        if True, the RMT approximation is used whe dfs are estimated and not provided.
    ...
    def estimate_df(self,S):
        """
        Estimates the degree of freedom (shape parameter) of the t- Wishart distribution
        with unknown center matrix and based on the provided samples
    ...
    def compute_class_center(self,S,df):
        """
        Compute the Maximum Likelihood Estimator (MLE) for the center matrix of the t- Wishart
        distribution with knowm parameters n and df, based on provided samples
    ...
    def _predict_discimination(self, covtest):
        ...
        discrimination : array, shape (n_trials,n_classes)
            discrimination table between the testing SPD matrix and the centroids.
...
class SPD(RiemannianSubmanifold):
    """
    Class of SPD manifold, endowed with the Fisher Information Metric of the form : 
        g_S(A,B) = alpha*tr(AS^{-1}BS^{-1})+beta*tr((AS^{-1})tr(BS^{-1})
    where S is a SPD matrix and A,B are symmetric matrices (on the tangent space of S)
    ...
    def euclidean_to_riemannian_gradient(self, point, euclidean_gradient):
        """
        Computes the Riemannian gradient of a function at a given point of the manifold
        from its Euclidean gradient at this point.
...
def log_generator_density(n,p,df,neglect_df_terms=False):
    """
    Returns the log of the density generator function of the 
    t-Wishart distribution
...
def kurtosis_estimation(samples,n,center,rmt=False):
    ...
    rmt : bool, optional.
        If true, the RMT correction is applied in the estimation process. The default is False.
